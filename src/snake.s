...
move_to:
	CMP byte [snake+ecx+2],'u'
	JE move_to_up

	CMP byte [snake+ecx+2],'l'
	JE move_to_left

	CMP byte [snake+ecx+2],'d'
	JE move_to_down

	CMP byte [snake+ecx+2],'r'
	JE move_to_right
...
random_pos_apple:
	MOV eax,355
	MOV ebx,apple_pos_buf
	MOV ecx,2
	MOV edx,0
	INT 0x80

	MOV al,[apple_pos_buf]
	XOR ah,ah
	DIV byte [width]
	MOV [apple],ah

	MOV al,[apple_pos_buf+1]
	XOR ah,ah
	DIV byte [height]
	MOV [apple+1],ah
